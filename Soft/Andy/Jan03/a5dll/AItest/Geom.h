...
inline bool DoesIntersect( const SBound &a, const SBound &b )
{
	...
	if ( fabs2(ptDif) > sqr( a.s.fRadius ) + sqr( b.s.fRadius ) )
		return false;
	if ( 
...
{
	float x, y, z;
	x = _11*pt.x + _21*pt.y + _31*pt.z;
	y = _12*pt.x + _22*pt.y + _32*pt.z;
	z = _13*pt.x + _23*pt.y + _33*pt.z;
	pResult->x = x; pResult->y = y; pResult->z = z;
}
