...
void CGlobalAck::RemoveInvisibleSequences( IPlayer *pPlayer )
{
	...
	if ( !sequences.empty() )
	{
		...
		for ( list< CDBPtr<NDb::CDBAck> >::iterator i = sequences.begin(); i != sequences.end(); )	
		{
...
NDb::CDBAckSequence *CGlobalAck::GetSequence( IPlayer *pPlayer )
{
	...
	if ( !sequences.empty() )
	{
		...
		float fProb = 0;
		CRoulette roulette;
		for ( list< CDBPtr<NDb::CDBAck> >::iterator i = sequences.begin(); i != sequences.end(); ++i )
		{
			fProb += (*i)->fProbability;
			roulette.AddSector( (*i)->fProbability );
		}
		//
		...
		{
			list< CDBPtr<NDb::CDBAck> >::iterator i = sequences.begin();
