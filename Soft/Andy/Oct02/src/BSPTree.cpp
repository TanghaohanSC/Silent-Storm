...
struct SBSPPoly
{
		...
		vector<SAddedEdge> *pAddedEdgesL, vector<SAddedEdge> *pAddedEdgesR, vector<SAddedEdge> *pDangerousEdges ) const
	{
		...
		do
		{
			...
			{
				// ребро полностью лежит на плоскости. сверим его направление с направлением, образующим правую тройку 
				if ( bNowLeft )
				{
			...
			{
				// если старт не лежит на плоскости. добавляем его куда надо и идем к след. ребру
	...
	const CVec3 &v1, const CVec3 &v2, const SBSPGeometry &mesh, CVec3 *pVec )
{
	...
	for ( SAnglePoint *it = &flatPoints[0]; it != &flatPoints[nPoints]; ++it )
		if ( it->fAngle > 0 )
			it->fAngle -= FP_PI;
	// определяем стоимость для каждой из точек
	...
	for ( int i = 1; i < mesh.polygons.size(); ++i )
	{
		...
		for ( int k = 0; k < i; ++k )
		{
			...
			if ( fabs2(norm) < fBSPEpsilon * fBSPEpsilon )
			{
				...
				// проверка на то, что результат будет торчать наружу
