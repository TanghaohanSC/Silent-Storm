...
bool CUnitSel::Move( const CVec3 &ptMove )
{
	...
	CTPoint<int> pt = pUnit->ptPos + CTPoint<int>( Float2Int( move2.x ), Float2Int( move2.y ) );
	int nFloor = Min( 4.0f, pUnit->nFloor + ptMove.z / NBuilding::WALL_HEIGHT );
	//
...
bool CUnitSel::TrackMove( bool bTileAlign, const CVec3 &ptMove, const CVec2 &ptCursor )
{
	...
	CVec3 pt = CVec3( pUnit->ptPos.x, pUnit->ptPos.y, 0 ) + FP_INV_GRID_STEP * ptMove;
	ptLocal.x = FP_GRID_STEP * (Float2Int( pt.x ) - pUnit->ptPos.x);
	ptLocal.y = FP_GRID_STEP * (Float2Int( pt.y ) - pUnit->ptPos.y);
