...
////////////////////////////////////////////////////////////////////////////////////////////////////
void CBuildingSchema::AddNode( NDb::CRPGArmor *pArmor, const CVec3 &pt, bool bCellarWall, float fWeight, const vector<SJunction> &points )
{
	fWeight *= 0.1f;
	for ( int i = 0; i < points.size(); ++i )
	{
		CRod *p = AddRod( pArmor, SRodEdge( pt, true, fWeight ), SRodEdge( points[i].pt, false, 0 ), bCellarWall, points[i].bGround );
		if ( p )
		{
...
//	ComputeStabilityR();
	...
	for ( vector<CJunction>::iterator pJ = juncs.begin(); pJ != juncs.end(); ++pJ )
	{
		...
		else if ( pJ->GetStability() == CJunction::STABLE )
		{
			...
			for ( int i = XP; i < UP; ++i )
			{
				if ( pJ->IsLinkBroken( EDirection( i ), nHP ) )
				{
