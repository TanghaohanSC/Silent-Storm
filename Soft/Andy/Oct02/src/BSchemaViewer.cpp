...
public:
	vector<CObj<NGScene::CRenderNode> > pNodes;
	vector<CObj<NGScene::CPolyline> > pLines;
	vector<CObj<NGScene::CText> > pStrings;
};
////////////////////////////////////////////////////////////////////////////////////////////////////
CObjectBase* ViewBuildingSchema( NGScene::IGameView *pScene, int nBuildingID, NBuilding::CBuildingGrid *pBGrid, const SFBTransform &pos )
{
	...
	if ( !pBInfo )
		return 0;
	return ViewBuildingSchema( pScene, pBInfo, pBGrid, pos );
}
////////////////////////////////////////////////////////////////////////////////////////////////////
CObjectBase* ViewBuildingSchema( NGScene::IGameView *pScene, NBuilding::CBuildInfo *pBInfo, NBuilding::CBuildingGrid *pBGrid, const SFBTransform &pos )
{
	/*
	for ( int i = 0; i < 30; ++i )
	{
...
*/
	...
	NBuilding::MakeBuildingSchema( pSchema, pBGrid, pBInfo );
	pSchema->Start();
	int nIterations = 0;
	if ( nIterations )
	{
	...
	for ( vector<NBuilding::CJunction>::const_iterator pJ = juncs.begin(); pJ != juncs.end(); ++pJ )
	{
		...
		switch ( st )
		{
			...
			case NBuilding::CJunction::STABLE:
				...
				if ( pJ->IsCellarWall() )
					color = CVec4( 0.35f, 0.4f, 0.35f, fAlpha );
				break;
...
//				break;
	...
	//
	CVec4 lcr( 0.2f, 0.7f, 0.7f, fAlpha );
	for ( vector<NBuilding::CRod>::const_iterator pR = rods.begin(); pR != rods.end(); ++pR )
	{
		const NBuilding::CRod &rod = *pR;
		...
		CVec4 color = fM > 40 ? CVec4( 1, 1, 1, fAlpha ) : fScale * sqrt( fM ) * lcr;
		color.w = fAlpha;
