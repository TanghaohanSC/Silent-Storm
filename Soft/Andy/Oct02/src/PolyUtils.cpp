...
{
	NStr::DebugTrace( "(%.2f %.2f)-(%.2f %.2f)", sEdge.iBegPoint->vPoint.x, sEdge.iBegPoint->vPoint.y, sEdge.iEndPoint->vPoint.x, sEdge.iEndPoint->vPoint.y );
	NStr::DebugTrace( " Flags: " );
	if ( sEdge.nFlags & CLIP_MARK )
		NStr::DebugTrace( "MARK " );
	if ( sEdge.nFlags & CLIP_INSIDE )
		NStr::DebugTrace( "INSIDE " );
	if ( sEdge.nFlags & CLIP_OUTSIDE )
		NStr::DebugTrace( "OUTSIDE " );
	if ( sEdge.nFlags & CLIP_SHARED_SAME )
		NStr::DebugTrace( "SHARED_SAME " );
	if ( sEdge.nFlags & CLIP_SHARED_TOWARDS )
		NStr::DebugTrace( "SHARED_TOWARDS " );
...
//// DEBUG
	...
	{
		case OP_SUB:
			NStr::DebugTrace( "Op SUB\n" );
			break;
		case OP_INTERSECTION:
			NStr::DebugTrace( "Op INT\n" );
			break;
	}
		...
		else if ( bSet && ( iTempLink->fAngle != iEdgeLink->fAngle ) )
		{
			...
			if ( EdgeRule( eOp, iTempLink->eType, *( iTempLink->iEdge ), &eTempDir ) )
			{
				...
				*piEdge = iTempLink->iEdge;
				...
				NStr::DebugTrace( "Found (%s) ", iTempLink->eType == POLY_SOURCE ? "source" : "clip" );
				DumpEdge( *( iTempLink->iEdge ) );
				return;
			}
			NStr::DebugTrace( "Skiped (%s) ", iTempLink->eType == POLY_SOURCE ? "source" : "clip" );
		}
	...
	NStr::DebugTrace( "Source:\n" );
	for ( list<SPolygon>::iterator iTemp = listPolygons.begin(); iTemp != listPolygons.end(); iTemp++ )
		DumpPolygon( *iTemp );
	NStr::DebugTrace( "Clip:\n" );
	for ( list<SPolygon>::iterator iTemp = listClipPolygons.begin(); iTemp != listClipPolygons.end(); iTemp++ )
		DumpPolygon( *iTemp );
	NStr::DebugTrace( "ERROR: Can't find jump-link for point %f %f\n", iPoint->vPoint.x, iPoint->vPoint.y );
	//ASSERT( 0 );
}
////////////////////////////////////////////////////////////////////////////////////////////////////
