...
const int NUM_ANGLES = 14;
SAngle angles[NUM_ANGLES] = {
...
*/
	...
	for ( int nP = 0; nP < NUM_PARTICLES; ++nP )
	{
		...
		SBonePose bone = defaultPose[nBone];
		bone.pos += shifts[nP];
		// CRAP{
		//if ( nP == 3 )
		//	bone.pos.x += 0.1f;
	...
	for ( int nP = 0; nP < NUM_PARTICLES; ++nP )
	{
		...
		last[nP] = bone.pos;
		if ( nP > 14 )
			last[nP] = (last[nP-10] + last[nP-8]) / 2;
		//last[nP] = particles[nP];
		// impulse
		/*
		if ( nP < 5 )
		{
			last[nP].x -= imp.x * DELTA_T;
			last[nP].y -= imp.y * DELTA_T;
			last[nP].z -= imp.z * DELTA_T;
		}
		*/
	}
...
void CParticleSkeleton::GetFrame( STime t, SSkeletonPose *pPose )
{
	...
	{
		SSkeletonPose pose, nextPose;
		pose.resize( pPose->size() );
		nextPose.resize( pPose->size() );
		pInput->GetFrame( tActive, &pose );
		pInput->GetFrame( tActive + DELTA_T_DWORD, &nextPose );
		Init( tActive + DELTA_T_DWORD, pose, nextPose, impact );
	}
...
void CParticleSkeleton::Step()
{
	...
	results.clear();
	NAI::CollideSliding( pMap, spheres, velocities, &results, NWorld::TS_PASS_BLOCKER );
	for ( int nP = 0; nP < nParticles; ++nP )
...
*/
		...
		if ( nStep > N_INCREMENT_STEP )
			fStopDelta = F_STOP_DELTA + (nStep - N_INCREMENT_STEP) * F_STOP_DELTA_INCREMENT;
		bStopped = true;
		for ( int nP = 0; nP < nParticles; ++nP )
		{
...
////////////////////////////////////////////////////////////////////////////////////////////////////
int CParticleSkeleton::operator&( CStructureSaver &f )
{
...
struct SCollision
{
	NAI::CCollider::SCollisionPoint res;
	CVec3 vel;
	float fPart;
	CVec3 center;
};
...
void CASphereSet::Step()
{
	...
	if ( !pMap->CalcIntersection( check.ptCenter, check.fRadius, NWorld::TS_PASS_BLOCKER ) )
	{
	...
	while ( fTimeLeft > 1e-4f && nCycle++ < 32 )
	{
		...
		//NStr::DebugTrace( "Timeleft: %f  Step: %f  Energy: %f\n", fTimeLeft, fTimeStep, fEnergy );
		vector<SSphere> poss;
		...
		NAI::CollideInfo( pMap, poss, vels, &ress, NWorld::TS_PASS_BLOCKER );
