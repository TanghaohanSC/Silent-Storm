...
const float F_MIN_CLIMB_HEIGHT = 0.36f;
...
struct STile
{
	...
	char nDisplacement;
	char nFlags;
	char nFlipper; // (nFlipper != 0) if a tile is locked by a door or another flipping object and is a number of that flipper + 1
...
public:
	...
	ZEND int operator&( CStructureSaver &f ) { f.Add(3,&nFirstFloor); f.Add(4,&ptOrigin); f.Add(5,&ptXDir); f.Add(6,&squareLevel); f.Add(7,&squareTrackers); f.Add(8,&pNet); f.Add(9,&layers); f.Add(10,&nXSize); f.Add(11,&nYSize); f.Add(12,&bFreeze); f.Add(13,&ignoreRects); return 0; }
	CLayersGroup() :bFreeze(false) {}
...
public:
	...
	{
		ZDATA
		vector<bool> pointPassable;
		vector<bool> pointOnUpperHalf;
		vector<char> nLocks;
...
public:
	struct SFlipper
	{
		...
		bool bOpen;
		ZEND int operator&( CStructureSaver &f ) { f.Add(2,&nFlipper); f.Add(3,&locksOpen); f.Add(4,&locksClosed); f.Add(5,&bOpen); return 0; }
		SFlipper() {}
	};
...
private:
	...
	bool bFreeze;
	ZEND int operator&( CStructureSaver &f ) { f.Add(2,&pMap); f.Add(3,&layers); f.Add(4,&lockedPlaces); f.Add(5,&dynLockedPlaces); f.Add(6,&bColouringConstructed); f.Add(7,&oldAccountPoints); f.Add(8,&pWaysCalcer); f.Add(9,&flippers); f.Add(10,&flippersHash); f.Add(11,&bFreeze); f.Add(12,&groups); return 0; }
	...
	void GetLockArea( vector<SPathPlace> *pRes, const SPathPlace &p ) const;
	void LockUnlockPoint( const SPathPlace &p, bool bLock );
	void LockUnlockDynamic( const SPathPlace &p, bool bLock );
...
public:
	CPathNetwork( IAIMap *_pMap = 0 ): pMap(_pMap), bFreeze(false) {}
	...
	virtual bool IsLocked( const SPathPlace &p ) const;
	...
	//virtual void ForceLayersRecalc( const SSphere &s );
	//virtual ETransitionType GetTransitionType( const SPathPlace &from, const SPathPlace &to );
	virtual void CreateLadder( int nX, int nY, int nHeight, int nRotation, int nLayersGroup, int nFloor );
	virtual void UpdateColouring( const vector<SPathPlace> &lockers );
	...
	SFlipper *GetFlipper( CPtr<CObjectBase> pSrc );
	...
	virtual SPathPlace GetDeployPlace( const SPathPlace &cur, bool bFirst );
