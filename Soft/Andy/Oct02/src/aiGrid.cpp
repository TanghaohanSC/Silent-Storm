...
public:
	...
	CTPoint<int> p;
	SBound bv;
	ZEND int operator&( CStructureSaver &f ) { f.Add(2,&pLayersGroup); f.Add(3,&p); f.Add(4,&bv); return 0; }

	CLayersSetTracker() {}
	CLayersSetTracker( IAIMap *pAIMap, CLayersGroup *_pLayersGroup, const CTPoint<int> &_p, const SBound &_bv )
		: pLayersGroup(_pLayersGroup), p(_p), bv(_bv)
	{ 
		pAIMap->AddTracker( this, bv, NWorld::TS_PASS_BLOCKER|NWorld::TS_GO_OVER, false );
	}
...
public:
	...
	{
		pAIMap->AddTracker( this, _bv, NWorld::TS_PASS_BLOCKER|NWorld::TS_LADDER_PART, false );
	}
	virtual void OnChange() 
	{ 
		if ( !IsValid(pLayer) )
		...
		const CTRect<int> &region, char flag, char backFlag )
{
	...
	for ( int x = nMinX; x < nMaxX; ++x )
	{
		for ( int y = nMinY; y < nMaxY; ++y )
		{
			...
			if ( ( t1.nMoveHC & flag ) == 0 )
				continue;
			CNodesLayer::STile &t2 = tiles[ y + shift.y ][ x + shift.x ];
			CNodesLayer::STile &t3 = tiles[ y + shift.y * 2 ][ x + shift.x * 2 ];
			if ( t1.nHeight >= t2.nHeight )
				continue;
			if ( ( t2.nMoveHC & flag ) && ( t3.nHeight < t2.nHeight ) )
				continue;
			if ( ( t2.nMoveCrouch & flag ) && ( t3.nPassable & CP_CROUCH ) )
				continue;
			t1.nMoveHC &= ~flag;
			t2.nMoveHC &= ~backFlag;
		}
...
void CNodesLayer::RefreshLadder( int nLadder, IAIMap *pMap )
{
	...
	// отмечаем
	// разметка лестницы на верхнюю и нижнюю часть
	...
	if ( ladder.bConsistent ) // лестница полностью проходима
	{
	...
	else // лестница частично разрушена
	{
		int i;
		...
		for ( i = firstInpassable; i < lastInpassable; ++i )
			ladder.pointPassable[ i ] = false; // непроходимые точки отныне образуют сплошной отрезок
	}
	// create tracker
...
void CNodesLayer::AddNearPoints( const SSphere &s, vector<SPathPlace> *pRes, bool bTakeAll )
{
	...
	for ( int y = r.miny; y <= r.maxy; ++y )
	{
		for ( int x = r.minx; x <= r.maxx; ++x )
		{
			...
			if ( fabs2( s.ptCenter - GetCP( p ) ) < sqr( s.fRadius ) )
			{
				CNodesLayer::STile &t1 = tiles[ y ][ x ];
				bool bAdd = true;
				if ( !bTakeAll )
...
////////////////////////////////////////////////////////////////////////////////////////////////////
/*bool CNodesLayer::GetLadder(int *nLadder, int *_nLayer, const SPathPlace &pt)
{
	bool ret = false;
	SPathPlace pt1;
	pt1.nData = 0;
	pt1.nX = pt.nX;
	pt1.nY = pt.nY;
	pt1.nLayer = pt.nLayer;
	pt1.nIntegral = pt.nIntegral;
	STransitionSet &trans = transitions[pt1];
	list<SLink>::iterator i;
	for ( i = trans.links.begin(); i != trans.links.end(); ++i )
	{
		if ( !i->dst.nIntegral )
		{
			ret = true;
			*nLadder = i->dst.nX;
			*_nLayer = i->dst.nLayer;
		}
	}
//	for ( int j = 0; j < pNet->layers.size(); ++j )
//	{
//	for ( int i = 0; i < pNet->layers[j]->ladders.size(); ++i )
//		{
//			SLadder &l = pNet->layers[j]->ladders[i];
//			if ( ((l.placeOnBottom.nX == pt.nX) && (l.placeOnBottom.nY == pt.nY) && (l.placeOnBottom.nLayer == pt.nLayer)) || ((l.end.nX == pt.nX) && (l.end.nY == pt.nY) && (l.end.nLayer == pt.nLayer)) )
//			{
//				ret = true;
//				*nLadder = i;
//				*_nLayer = j;
...
void CPathNetwork::FinishGridConstruction()
{
	...
	// используем этот цикл дважды, т.к. на первом проходе могли быть созданы новые слои, которые
	// соответственно не до конца посчитались. 
	for ( int i = 0; i < groups.size(); ++i )
	{
		CLayersGroup *pGroup = groups[i];
		int nMaxX = pGroup->nXSize;
...
////////////////////////////////////////////////////////////////////////////////////////////////////
void CPathNetwork::GetLockArea( vector<SPathPlace> *pRes, const SPathPlace &p ) const
{
	if ( p.GetPose() != CM_LAY )
	...
	else // LAY pose check
	{
		...
		GetLockAreaInternal( pRes, p, -nDX, -nDY );
		if ( nDX != 0 && nDY != 0 ) // diagonal lay-pose lock
		{
...
////////////////////////////////////////////////////////////////////////////////////////////////////
bool CPathNetwork::IsLocked( const SPathPlace &p ) const
{
	int nX, nY;
	if ( p.IsIntegral() )
	{
		nX = p.GetX();
		nY = p.GetY();
		return layers[p.GetLayer()]->GetLocks( nX, nY ) != 0;
	}
	else
	{
...
////////////////////////////////////////////////////////////////////////////////////////////////////
void CPathNetwork::LockUnlockDynamic( const SPathPlace &p, bool bLock )
{
	vector<SPathPlace> res;
	GetLockArea( &res, p );
...
////////////////////////////////////////////////////////////////////////////////////////////////////
void CPathNetwork::UpdateColouring( const vector<SPathPlace> &lockers )
{
...
////////////////////////////////////////////////////////////////////////////////////////////////////
CPathNetwork::SFlipper *CPathNetwork::GetFlipper( CPtr<CObjectBase> pSrc )
{
...
////////////////////////////////////////////////////////////////////////////////////////////////////
SPathPlace CPathNetwork::GetDeployPlace( const SPathPlace &cur, bool bFirst )
{
	//if ( !bFirst )
	//	cur.SetXY( cur.GetX(), cur.GetY() + 2 );
	//return;
	SPathPlace res( cur );
	CVec3 cp( GetCP( cur ) );
	...
	if ( !bFirst )
	{
		...
		cp.y += vDir.x * 1.5f;
		SPosition pos;
		SetOnLayer( &pos, cur.GetLayer(), cp );
		res = pos.p;
	}
	EDirection dir = GetClosestDir( cur.GetLayer(), fAngle );
