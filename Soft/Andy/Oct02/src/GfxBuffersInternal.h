...
public:
	...
	void Lock( DWORD dwFlags )
	{
		...
		if ( !pLocked )
		{
			...
			HRESULT hr = obj->Lock( 0, 0, &pLocked, dwFlags );
			ASSERT( hr == D3D_OK );
...
public:
	CVB() {}
	// size in bytes
	CVB( int _nSize, ETrueBufferUsage eUsage ): CRBase<IDirect3DVertexBuffer8>(_nSize)
	{
...
////////////////////////////////////////////////////////////////////////////////////////////////////
// large index buffer
class CIB: public CObjectBase
{
	OBJECT_NOCOPY_METHODS( CIB );
public:
	NWin32Helper::com_ptr< IDirect3DIndexBuffer8 > obj;
	unsigned char *pLocked;

	CIB() {}
	// size in bytes
	CIB( int _nSize, ETrueBufferUsage eUsage )
	{
...
{
	OBJECT_NOCOPY_METHODS( CIB16 );
public:
	CIB16() {}
	CIB16( int _nSize, ETrueBufferUsage eUsage ): CRBase<IDirect3DIndexBuffer8>(_nSize)
	{
		HRESULT hRes = pDevice->CreateIndexBuffer(
			nSize, 
			GetUsageFlags( eUsage ),
