...
// non-arrays
	...
	int CalcPoseInPoint( int x, int y, NAI::CCollider *pCollider, const CArray2D<unsigned short> &tempH, 
		CArray2D<STile> *pTemp, const CArray2D<char> &flags, int nLayer, int nDisplacement = 0 );
	...
	STile &GetFlipperTile( int x, int y, int nLayer, const SDoorColliderAnalyzer &analyzer, const STile &t, char *pNFlipper );
public:
