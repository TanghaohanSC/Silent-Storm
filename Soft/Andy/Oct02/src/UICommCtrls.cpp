...
const int
	...
	N_TOOLTIP_DEFAULT_WIDTH = 300;
////////////////////////////////////////////////////////////////////////////////////////////////////
// CEdit
////////////////////////////////////////////////////////////////////////////////////////////////////
CEdit::CEdit( const SWindowInfo &sInfo ):
	CWindow( sInfo ), nSize( 255 ), nCursor( 0 ), bActiveState( false ), bCursorVisible( false ), sFlashTime( 0 )
{
...
////////////////////////////////////////////////////////////////////////////////////////////////////
void CEdit::GetText( wstring *pwsString ) const
{
	*pwsString = wsText;
}
////////////////////////////////////////////////////////////////////////////////////////////////////
void CEdit::SetText( const wstring &wsString )
{
	wsText = wsString;
...
void CEdit::Draw( const STime &sTime, NGScene::I2DGameView *pView )
{
	if ( GetSize() != pSize->GetValue() )
		pSize->Set( GetSize() );
	if ( wsText != pTextString->GetValue() )
		pTextString->Set( wsText );
	...
	if ( !pText ) pText = pView->CreateText( pTextString, pSize, false );
	...
	case EVENT_TEMPLATECREATE:
		{
			...
			pWindow = AddState( 1 );
			pCheck = new CImage( SWindowInfo( pWindow, SPoint( 0, 0), GetSize(), "", STYLE_ENABLED | STYLE_VISIBLE | STYLE_TRANSPARENT ) );
			pCheck->SetImage( sEvent.pControl->pTextures[4] );
			break;
		}
	}
	...
	CWindow( sInfo )
{
	...
	pText->GetIML()->SetHandler( L"value", VALUEHandler, this );

	pBackground = new CImageDraw( SRect( 0, 0, GetSize().x, GetSize().y ) );
	pBackground->SetColor( NGfx::SPixel8888( 0x1F, 0x1F, 0x1F, 0xDF ) );
}
////////////////////////////////////////////////////////////////////////////////////////////////////
...
void CToolTip::Draw( const STime &sTime, NGScene::I2DGameView *pView )
{
	...
	pBackground->SetWindow( SRect( 0, 0, GetSize().x, GetSize().y ) );
	pBackground->Draw( this, sTime, pView );

	CWindow::Draw( sTime, pView );
}
////////////////////////////////////////////////////////////////////////////////////////////////////
void CToolTip::VALUEHandler( IMLLayout *pLayout, const vector<wstring> &paramsSet, void *pContext )
{
	if ( paramsSet.size() != 4 )
		return;
	...
	CToolTip *pToolTip = dynamic_cast<CToolTip*>( pBase );
	if ( !IsValid( pToolTip ) )
	{
		ASSERT( 0 );
		return;
	...
	hash_map<wstring,wstring>::const_iterator iTemp = pToolTip->valuesMap.find( paramsSet[3] );
	if ( iTemp != pToolTip->valuesMap.end() )
	{
		CPtr<IMLObject> pText = CreateIMLTextObject( iTemp->second );
		pLayout->AddObject( pText );
	}
}
////////////////////////////////////////////////////////////////////////////////////////////////////
// CSlider
...
CSlider::CSlider( const SWindowInfo &sInfo ):
	CWindow( sInfo ), nValue( 0 ), nMinValue( 0 ), nMaxValue( 100 ), bSlide( false )
{
...
////////////////////////////////////////////////////////////////////////////////////////////////////
void CSlider::SetValue( int nVal )
{
	nValue = min( max( nVal, nMinValue ), nMaxValue );
}
////////////////////////////////////////////////////////////////////////////////////////////////////
void CSlider::SetRange( int _nMinValue, int _nMaxValue )
{
	nMinValue = _nMinValue;
	nMaxValue = _nMaxValue;

	nValue = min( max( nValue, nMinValue ), nMaxValue );
}
////////////////////////////////////////////////////////////////////////////////////////////////////
...
void CSlider::Slide( int nX, int nY )
{
	...
	if ( GetStyle( SLDRSTYLE_HORZ ) )
		nValue = nMinValue + Float2Int( float( nX - sPosition.x - sSliderSize.x / 2 ) * float( nMaxValue - nMinValue ) / float( sSize.x - sSliderSize.x ) );
	else
		nValue = nMinValue + Float2Int( float( nY - sPosition.y - sSliderSize.x / 2 ) * float( nMaxValue - nMinValue ) / float( sSize.y - sSliderSize.y ) );
...
void CSlider::Update( const STime &sTime, NGScene::I2DGameView *pView )
{
	...
	if ( GetStyle( SLDRSTYLE_HORZ ) )
		pSlider->SetPosition( SPoint( nValue * ( sPadSize.x - sSliderSize.x ) / ( nMaxValue - nMinValue ), sSliderPosition.y ) );
	else
		pSlider->SetPosition( SPoint( sSliderPosition.x, nValue * ( sPadSize.y - sSliderSize.y ) / ( nMaxValue - nMinValue ) ) );
...
////////////////////////////////////////////////////////////////////////////////////////////////////
void CScroll::SetValue( int nVal )
{
	pSlider->SetValue( nVal );
}
////////////////////////////////////////////////////////////////////////////////////////////////////
void CScroll::SetRange( int _nMinValue, int _nMaxValue )
{
	pSlider->SetRange( _nMinValue, _nMaxValue );
}
////////////////////////////////////////////////////////////////////////////////////////////////////
...
{
	SItem sTempItem;
	if( FindItemByID( nID, &sTempItem ) )
	{
		ASSERT( 0 && "Item with same ID already exist" );
		return;
	}
...
void CListView::SetSelectedItem( int nID )
{
	...
	if ( IsValid( sSelected.pWindow ) )
		SendMessage( sSelected.pWindow, SEvent( EVENT_LISTVIEW_ITEMSELECTED, ELV_ITEMSSELECTED_FALSE ) );

	sSelected = sItem;
	SendMessage( sSelected.pWindow, SEvent( EVENT_LISTVIEW_ITEMSELECTED, ELV_ITEMSSELECTED_TRUE ) );
}
////////////////////////////////////////////////////////////////////////////////////////////////////
...
{
	pTexture = new NGScene::CBWScreenshotTexture();
	pTexture->Generate();
}
////////////////////////////////////////////////////////////////////////////////////////////////////
NGScene::CBWScreenshotTexture* CScreenShot::GetTexture() const
{
	return pTexture;
}
////////////////////////////////////////////////////////////////////////////////////////////////////
void CScreenShot::SetTexture( NGScene::CBWScreenshotTexture* _pTexture )
{
	pTexture = _pTexture;
}
////////////////////////////////////////////////////////////////////////////////////////////////////
void CScreenShot::Draw( const STime &sTime, NGScene::I2DGameView *pView )
{
	if ( IsValid( pTexture ) )
	{
		...
		const CTPoint<int> &sSSSize = pTexture->GetSize();

		CRectLayout sLayout;
		sLayout.scale = CTPoint<float>( pView->GetViewportSize().x / sSSSize.x, pView->GetViewportSize().y / sSSSize.y );
		sLayout.AddRect( 0, 0, CTRect<float>( 0, 0, sSSSize.x, sSSSize.y ) );
