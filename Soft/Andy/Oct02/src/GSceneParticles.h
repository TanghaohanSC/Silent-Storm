...
protected:
	struct SEffectInfo
	{
		...
		DWORD dwPColor;
		ZEND int operator&( CStructureSaver &f ) { f.Add(2,&pEffect); f.Add(3,&nStartParticle); f.Add(4,&lmPlacement); return 0; }

		SEffectInfo() {}
		SEffectInfo( CParticleEffect *_pEffect, int _nStartParticle, const SParticleLightInfo &lm, DWORD _dwPColor )
			: pEffect(_pEffect) , nStartParticle(_nStartParticle), lmPlacement(lm), dwPColor(_dwPColor) {}
	};
	ZDATA
...
public:
		...
		DWORD _dwPColor )
	{
		...
		effects.push_back( SEffectInfo( pEffect, nStartParticle, lm, dwPColor ) );
		pl = lm;
		bcPart.Clear();
		bIsAddingPart = true;
		dwPColor = _dwPColor;
...
public:
		...
		DWORD dwPColor )
	{
		...
		CBaseParticlesGeometry::Start( pEffect, nStartParticle, bv, lm, dwPColor );
		if ( !pWriteBuffer )
			pWriteBuffer = new SWriteBuffer( &pValue );
	}
