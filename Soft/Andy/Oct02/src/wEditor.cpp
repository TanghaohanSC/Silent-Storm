...
////////////////////////////////////////////////////////////////////////////////////////////////////
void CEditorTerrain::Update( int _nCutFloor )
{
...
void CEditorWorld::AddBuilding( const SMapBuilding &info )
{
	...
	NBuilding::BuildingHP( pBInfo, info.pGrid );
...
////////////////////////////////////////////////////////////////////////////////////////////////////
void CEditorWorld::CreateTerrain()
{
	ASSERT( IsValid( pVar ) );
	SMapInfo mapInfo;
	SRand rand;
...
//		pTerrainInfo.Refresh();
	...
	*/
	...
	pTerrain = new NWorld::CEditorTerrain( pShow, pTerrainInfo, pAIMap, pTime, mapInfo.nBaseTerrainFloor, mapInfo.holesList, mapInfo.wallsList, pBuildingGrid->GetCutFloor() );
}
////////////////////////////////////////////////////////////////////////////////////////////////////
void CEditorWorld::CreateRandom( int nVariantID, bool bBuildingStability, 
	const list< CPtr<NScenario::CScenarioClue> > &clues, int nMobsLevel, CObj<CPostWorldCreateInfo> *pPostInfo, SRandomSeed sSeed )
{
...
void CEditorWorld::UpdateWorld( STime tScene, IPlayer *pPlayer )
{
	...
	if ( bUpdateTerrain && (!rectsToUpdateGeometry.empty() || !rectsToUpdateTexture.empty()) );
		RebuildTerrInfo();
	...
	if ( pVar )
	{
		for ( int i = 0; i < pVar->terrainSpots.size(); ++i )
		{
			if ( !IsValid( pVar->terrainSpots[i] ) )
				continue;
			CPtr<NDb::CTerrainSpot> pSpot = pVar->terrainSpots[i]->CreateTerrainSpot( &rand );
			if ( !IsValid( pSpot ) )
				continue;
			NBuilding::SProjectedSpot spot;
			spot.nRotation = pSpot->nRotation;
			spot.nMaterialID = pVar->terrainSpots[i]->pSpot->GetRecordID();
...
void CEditorWorld::AddMapObjects()
{
	...
	for ( int i = 0; i < sMapBuildingInfo.pVariant->pFinalElements.size(); ++i )
	{
		...
		if ( !IsValid( p ) )
			continue;
		iobjects.push_back( IEditorObject::Create( this, p, nCutFloor ) );
	}
}
////////////////////////////////////////////////////////////////////////////////////////////////////
void CEditorWorld::AddFragments( const vector<NBuilding::SBuildFragment> &frags )
{
	...
	for ( int i = 0; i < frags.size(); ++i )
	{
		...
		if ( fr.nSubBlockID != NBuilding::GetPartHashID( 1, 1, 1 ) )
			continue;
...
////////////////////////////////////////////////////////////////////////////////////////////////////
void CEditorWorld::RebuildTerrInfo()
{
	SMapInfo mapInfo;
	if ( IsValid( pVar ) )
	{
		BuildTerrain( pVar->GetRecordID(), &mapInfo, GetUserSettings().GetSubTemplateDepth() + 1 );
		pTerrainInfo->GetWritableInfo() = mapInfo.terrain;
	}
}
////////////////////////////////////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////////////////////////////////////////
void CEditorWorld::UpdateGrass( const CTRect<float> &sRegion )
{
	CreateTerrain();
}
////////////////////////////////////////////////////////////////////////////////////////////////////
...
//					UpdateExplosions();
	...
	for ( int i = 0; i < pVar->pFinalElements.size(); ++i )
	{
		...
		if ( !IsValid( pF ) || pF->GetRecordID() != nDBObjectID )
			continue;
		iobjects.push_back( IEditorObject::Create( this, pF, nCutFloor ) );
		pAIMap->Sync();
		break;
	}
}
