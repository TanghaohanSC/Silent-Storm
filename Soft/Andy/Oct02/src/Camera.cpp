...
public:
	ZEND int operator&( CStructureSaver &f ) { f.Add(2,&fFOV); f.Add(3,&sPlacement); f.Add(4,&sLimits); f.Add(5,&sScreenRect); return 0; }
...
CBaseCamera::CBaseCamera():
	fFOV( F_FOV ), sScreenRect( 0, 0, 1, 1 ),
...
void CBaseCamera::GetTransform( CTransformStack *pTS, const CVec2 &vScreenSize ) const
{
	...
	pTS->MakeProjective( vScreenSize, fFOV, 0.1f, 100, CVec2( fShiftX, fShiftY ) );
...
{
	SHMatrix res;
	MakeMatrix( &res, sPlacement.fPitch, sPlacement.fYaw, GetCP() );
	return res;
}
////////////////////////////////////////////////////////////////////////////////////////////////////
...
void CMayaCamera::Update( const STime &sTime )
{
	...
	if ( fabs2( ptMove ) > FP_EPSILON )
	{
		...
		SHMatrix m;
		MakeMatrix( &m, sPlacement.fPitch, sPlacement.fYaw, GetCP() );
