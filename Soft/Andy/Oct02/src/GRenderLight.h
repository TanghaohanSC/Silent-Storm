...
////////////////////////////////////////////////////////////////////////////////////////////////////
struct SLightInfo;
extern bool bStaticShadowDepthRendered;
class CDirectionalLight: public ILight
{
	...
	float fMaxHeight;
	ZEND int operator&( CStructureSaver &f ) { f.Add(2,&pStaticTracker); f.Add(3,&pColor); f.Add(4,&pGlossColor); f.Add(5,&vShadowColor); f.Add(6,&pAmbient); f.Add(7,&vDepth); f.Add(8,&bLightmapOnly); f.Add(9,&vLightDir); f.Add(10,&mPrevCamera); f.Add(11,&fMaxHeight); return 0; }
	//
	void PrepareLightInfo( SLightInfo *pLightInfo );
	template<class TParam, class TOp>
	void PrepareRender( CRenderCmdList *pRes, ESceneRenderMode renderMode, 
		const vector<SRenderFragmentInfo*> &fragments, TOp process, const TParam &param )
	{
		for ( vector<SRenderFragmentInfo*>::const_iterator i = fragments.begin(); i != fragments.end(); ++i )
		{
			SRenderFragmentInfo &f = **i;
			SOpGenContext op( &pRes->ops, &f );
			const SMaterialInfo &info = f.pStatic->pMaterial->GetMaterialInfo();
			process( op, info, param, renderMode );
		}
	}
	template<class TParam, class TOp>
	void Render( CTransformStack *pTS, NGfx::CRenderContext *pRC, ESceneRenderMode renderMode, 
		IRender *pRender, CSceneFragments &scene, const SLightInfo &lightInfo, TOp process,
		const TParam &param )
	{
		CRenderCmdList lightOps;
		PrepareRender( &lightOps, renderMode, scene.GetSelected(), process, param );
		Execute( pRender, pRC, *pTS, lightOps, lightInfo );
	}
	void FinalPass( NGfx::CRenderContext *pRC, ESceneRenderMode renderMode );
	void RenderPPShadowOps( CTransformStack *pTS, CTransformStack *pClipTS, NGfx::CRenderContext *pRC, ESceneRenderMode renderMode, 
	...
	void RenderLightmaps( CTransformStack *pTS, NGfx::CRenderContext *pRC, IRender *pRender, CSceneFragments &scene, ESceneRenderMode renderMode );
	...
	void RenderInSinglePass( CTransformStack *pTS, CTransformStack *pClipTS, NGfx::CRenderContext *pRC, ESceneRenderMode renderMode, IRender *pRender, CSceneFragments &scene, const SParticleLMRenderTargetInfo &particleLM, const SLightInfo &lightInfo );
public:
	CDirectionalLight() {}
	CDirectionalLight( CFuncBase<CVec3> *pColor, CFuncBase<CVec3> *pGlossColor, 
		...
		CVersioningBase *_pStaticTracker, CFuncBase<CVec3> *pAmbient, bool bLightmapOnly );
	int GetPriority() const { return 2; }
	void Render( CTransformStack *pTS, CTransformStack *pClipTS, NGfx::CRenderContext *pRC, ESceneRenderMode renderMode, 
		IRender *pRender, CSceneFragments &scene, const SParticleLMRenderTargetInfo &particleLM );
	struct SRadianceInfo
	{
		bool bIsRendered;
		CVec3 vColor, vAmbientColor;
		CVec3 vDirection;
	};
	void GetRadianceInfo( SRadianceInfo *pRes, ESceneRenderMode rm );
};
////////////////////////////////////////////////////////////////////////////////////////////////////
class CPointLight: public ILight
{
	...
	void Add( SOpGenContext &op, ESceneRenderMode rm, const SMaterialInfo &info );
...
public:
	...
	virtual bool Update( NGfx::CRenderContext *pRC, ESceneRenderMode renderMode, IRender *pRender );
	virtual bool CheckCulling( CTransformStack *pTS );
