...
////////////////////////////////////////////////////////////////////////////////////////////////////
CVec3 CalcAverageColor( CFuncBase<STerrainInfo> *_pInfo, const CTRect<int> &sRegion )
{
	CDGPtr<CFuncBase<STerrainInfo> > pInfo(_pInfo);
	pInfo.Refresh();
	const STerrainInfo &info = pInfo->GetValue();
	CVec4 vAverage(0,0,0,0);
	// average tile color
	{
		...
		while( iTempTile.MoveNext() )
		{
			...
			if ( !pTile->pTexture )
				continue;
			unsigned char nTileIndex = pTile->GetRecordID();
			int nCount = 0;
			for ( int y = sRegion.y1; y < sRegion.y2; ++y )
			{
				for ( int x = sRegion.x1; x < sRegion.x2; ++x )
				{
					if ( info.typeMap[y][x] == nTileIndex )
						++nCount;
				}
			}
			vAverage += nCount * NGfx::GetCVec4Color( pTile->pTexture->dwAverageColor );
			nTotal += nCount;
		}
		if ( nTotal )
			vAverage /= nTotal;
	}
	float fTotalSize = FP_GRID_STEP * FP_GRID_STEP * sRegion.Width() * sRegion.Height();
	// account spots
	for ( int k = 0; k < info.spots.size(); ++k )
	{
		const STerrainSpot &s = info.spots[k];
		float fX = s.ptPos.x * ( 1 / FP_GRID_STEP );
		if ( fX < sRegion.x1 || fX > sRegion.x2 )
			continue;
		float fY = s.ptPos.y * ( 1 / FP_GRID_STEP );
		if ( fY < sRegion.y1 || fY > sRegion.y2 )
			continue;
		if ( !IsValid( s.pMaterial->pTexture ) )
			continue;
		CVec4 vColor = NGfx::GetCVec4Color( s.pMaterial->pTexture->dwAverageColor );
		float fArea = Min( fTotalSize, s.ptSize.x * s.ptSize.y * 0.5f );
		float fFraction = fArea / fTotalSize;
		vAverage = vAverage * ( 1 - fFraction ) + vColor * fFraction;
	}
	// account grass
	for ( int k = 0; k < info.grass.size(); ++k )
	{
		const SGrassLayer &g = info.grass[k];
		if ( g.nGrassID == -1 )
			continue;
		NDb::CGrass *pGrass = NDb::GetGrass( g.nGrassID );
		if ( !IsValid( pGrass ) )
			continue;
		int nTotal = 0;
		for ( int y = sRegion.y1; y < sRegion.y2; ++y )
		{
			for ( int x = sRegion.x1; x < sRegion.x2; ++x )
				nTotal += (int)g.grass[y][x];
		}
		float fGrass = ((float)nTotal) / sRegion.Width() / sRegion.Height() / 255 * g.fMaxDensity / FP_GRID_STEP / FP_GRID_STEP;
		float fGrassStrength = 0.25 * pGrass->ptScale.x * pGrass->ptScale.y * fGrass;
		float fFraction = Min( 1.0f, fGrassStrength );
		vAverage = vAverage * ( 1 - fFraction ) + CVec4(0.3f, 1, 0.3f,0 ) * fFraction;
	}
	return CVec3( vAverage.x, vAverage.y, vAverage.z );
}
////////////////////////////////////////////////////////////////////////////////////////////////////
...
static CTerrainTexturesCache cache256( 50, 256 );
#else
static CTerrainTexturesCache cache128( 200, 128 );
static CTerrainTexturesCache cache256( 50, 256 );
#endif
////////////////////////////////////////////////////////////////////////////////////////////////////
...
void CTerrainTexture::Recalc()
{
	...
	if ( fElapsedTime > F_LIMIT_TIME_RECALC || ( bIsStress && bBumpTexture ) || HasFileRequestsInFly() )
	{
