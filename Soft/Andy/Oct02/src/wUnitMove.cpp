...
class CExecMove: public CCommandExecute, public IExecMove
{
	...
	ZEND int operator&( CStructureSaver &f ) { f.Add(1,(CCommandExecute*)this); f.Add(2,&commandsQueue); f.Add(3,&pCurCmd); f.Add(4,&bLastCommand); f.Add(5,&result); f.Add(6,&bWaiting); f.Add(7,&bAfterWaiting); f.Add(8,&cTimeToWait); f.Add(9,&posToWait); f.Add(10,&desired); f.Add(11,&eParams); return 0; }
	...
	NRPG::EAction GetAction( const NAI::SUnitPosition &dst ) const
	{
		...
		return pUS->GetWorld()->GetMoveActionType( pUS->GetPosition(), dst, animator.IsCarryingCorpse() );
	}
	...
	bool TestDynamicMove( CCmdMove *pPrevMove )
	{
		CUnitAnimator &animator = pUS->animator;
		//ASSERT( pUS->CanDoGameMove( pPrevMove->pos ) );
		if ( !pUS->CanDoGameMove( pPrevMove->pos ) ) // now with new locks system it's nearly impossible
		{
			//result = FAILED;
			//pCurCmd = 0;
			//commandsQueue.clear();
...
public:
	CExecMove() {}
	CExecMove( CUnitServer *_pUS, NAI::CPath *pPath, NAI::EFindPathParams _eParams, ENeedActiveItem eActive );
...
// CExecMove
////////////////////////////////////////////////////////////////////////////////////////////////////
CExecMove::CExecMove( CUnitServer *_pUS, NAI::CPath *pPath, NAI::EFindPathParams _eParams, ENeedActiveItem eActive )
: CCommandExecute(_pUS), result(FINISHED), bWaiting(false), eParams(_eParams)
{
...
void CExecMove::DoCommand()
{
	...
	if ( CDynamicCast<CCmdMove> pPrevMove( pCurCmd ) )
	{
		if ( !bAfterWaiting )
		{
			// we are in movement mode, perform move itself, might be interrupted in this moment
			if ( !TestDynamicMove( pPrevMove ) )
			{
				return;
			}
	...
	pUS->GetWorld()->AddActionLocator( new CActionLocator( CActionLocator::TYPE_UNIT, pUS, position.GetCP() ) );
	// process it
...
int CExecMove::GetStartAP() const
{
	for ( list<CObj<CCommand> >::const_iterator i = commandsQueue.begin(); i != commandsQueue.end(); ++i )
	{
		if ( CDynamicCast<CCmdTravel> p( *i ) )
			return pUS->GetActionAP( GetAction( p->pos ) );
	}
	return 0;
}
////////////////////////////////////////////////////////////////////////////////////////////////////
...
///		pPath->DebugOutput();
	...
	//
/*	if ( pPlayer->IsCheatEnabled( CHEAT_MOVE ) )
	{
		NAI::SUnitPosition sPos = GetPosition();
		sPos.pos.p = pPath->points.back();
		commandsQueue.push_back( new CCmdTeleport( this, sPos ) );
		pCurrentPath = 0;
		return;
	}*/
	//
...
public:
	...
	{
		NRPG::EAction action = pWorld->GetMoveActionType( currentPos, _pos, bCorpse );
...
CCommandExecute* CreateSimpleMoveExecutor(
	CUnitServer *_pUS, NAI::CPath *pPath, NAI::EFindPathParams _eParams, ENeedActiveItem eActive )
{
	if ( !_pUS->GetUnitRPG()->CanMove() )
		return 0;
	return new CExecMove( _pUS, pPath, _eParams, eActive );
}
////////////////////////////////////////////////////////////////////////////////////////////////////
}
