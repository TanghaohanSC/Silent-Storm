...
void CFileTexture::Recalc()
{
	...
	switch ( pTex->type )
	{
		...
		case NDb::CTexture::TEXTURE_TRANSPARENT: eUsage = NGfx::TRANSPARENT_TEXTURE; break;
	...
	if ( !pRequest->IsReady() )
	{
		...
		pValue = NGfx::MakeTexture( 1, 1, 1, 
			NGfx::SPixel8888::ID, eUsage, 
			GetKey().bWrap ? NGfx::WRAP : NGfx::CLAMP );
		NGfx::CTextureLock<NGfx::SPixel8888> lock( pValue, 0, NGfx::INPLACE );
	...
	if ( file->GetSize() > 0 )
	{
		...
		pValue = NGfx::MakeTexture( hdr.nSizeX, hdr.nSizeY, hdr.nNumMipLevels, 
			hdr.format, eUsage, 
			GetKey().bWrap ? NGfx::WRAP : NGfx::CLAMP );
