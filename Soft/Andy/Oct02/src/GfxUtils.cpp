...
void C2DQuadsRenderer::Flush()
{
	...
	else
	{
		...
		case QRM_SOLID:
			{
				SEffectColorTexU color;
				rc.SetEffect( &color );
			}
			break;
		case QRM_BLUR_LIGHT:
			{
				NGfx::SEffShadowBlur blur;
				blur.vShift = CVec2( 1.5f * N_UV_PER_TEXEL_STEPS * fScaleU, 1.5f * N_UV_PER_TEXEL_STEPS * fScaleV );
				blur.fScaleU = fScaleU;
				blur.fScaleV = fScaleV;
				blur.pTex = pPrevContainer;
				rc.SetEffect( &blur );
			}
			break;
		default: ASSERT(0); break;
...
void CopyTexture( const NGfx::CRenderContext &_rc, const CVec2 &vTargetViewport, const CTRect<float> &rTarget, 
	NGfx::CTexture *pTex, const CTRect<float> &rSrc, const CVec4 &vColor )
{
...
void BlurLight( NGfx::CRenderContext *pRC, int nSrcRegister, int nDestRegister )
{
	...
	NGfx::C2DQuadsRenderer qr( *pRC, CVec2( rDest.x2, rDest.y2 ), QRM_BLUR_LIGHT );
