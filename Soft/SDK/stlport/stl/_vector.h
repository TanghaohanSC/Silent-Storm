...
protected:
  ...
  void _M_insert_overflow(pointer __position, const _Tp& __x, const __true_type&, 
	...
	// handles insertions on overflow
	void _M_insert_overflow1(pointer __position, const __false_type&, 
		size_type __fill_len, bool __atend = false) {
			const size_type __old_size = size();
			const size_type __len = __old_size + (max)(__old_size, __fill_len);
		...
		void _M_insert_overflow1(pointer __position, const __true_type&, 
			size_type __fill_len, bool __atend = false) {
				const size_type __old_size = size();
				const size_type __len = __old_size + (max)(__old_size, __fill_len);
				...
				pointer __new_finish = (pointer)__copy_trivial(this->_M_start, __position, __new_start);
				// handle insertion
				__new_finish = fill_n(__new_finish, __fill_len, _Tp());
				if (!__atend)
					// copy remainder
					__new_finish = (pointer)__copy_trivial(__position, this->_M_finish, __new_finish);
				_M_clear();
...
public:
  ...
  bool empty() const            { return this->_M_start == this->_M_finish; }
	...
	reference operator[](size_type __n) { ASSERT(__n < size() ); return *(begin() + __n); }
  const_reference operator[](size_type __n) const { ASSERT(__n < size() ); return *(begin() + __n); }
  ...
  explicit vector(size_type __n)
    ...
    this->_M_finish = uninitialized_fill_n(this->_M_start, __n); 
  }
...
#ifdef CHEAT_ASSERT_STL_11111
#undef ASSERT
#undef CHEAT_ASSERT_STL_11111
#endif
